...
//
// The way it could happen HERE is worth naming, because it is not the obvious one. This
// repository holds the texts an organisation accepts — the licence draft and, later, the
// entitlement terms. The accepted-terms half of the private store is a set of those same
// texts, filed by version against the purchases that accepted them. The text is public; the
// filing is evidence, and evidence of who accepted what is the private half. A terms
// directory would therefore look entirely at home here, which is why the gate keys on the
// store's own path shapes rather than on a word that has honest uses.
//
...
// publishable repositories — the website, the contracts, the registry and this one. They
...
if (selfTestFailure) {
  problem(selfTestFailure);
  process.exit(1);
}

const root = rootArgument(process.argv.slice(2), (m) => {
  problem(m);
  process.exit(1);
});
...
for (const f of findings) {
  problem(
    `${f.rel} is ${f.why} The store is never in a repository that is or becomes public; move ` +
      'it to the private ops records area and remove it here.',
  );
}

if (!root) {
  const missing = missingLandmarks(top, ['package.json', 'drafts', 'canonical', 'copy-lint']);
  if (missing.length > 0) {
    problem(
...
if (problems > 0) {
  console.error(`\nno-records gate: ${problems} problem(s).`);
  process.exit(1);
}

console.log(`no-records gate clean — ${visited} path(s) walked, none from the private recording store.`);
